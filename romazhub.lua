...
peallib = 'https://raw.githubusercontent.com/1AreYouMental110/LinoraLib-Mobile/PealLib/'
...
_mainFrame = Window and Window.Holder or nil

 HomeTab = Library:CreateHomeTab(Window, {
...
GriefAura = AurasGroup:AddToggle('GriefAura', {
    Text = 'Grief Aura',
    Default = false,
    Tooltip = 'Automatically delete blocks around target'
})

BlockAura = AurasGroup:AddToggle('BlockAura', {
    Text = 'Block Aura',
    Default = false,
    Tooltip = 'Automatically place blocks around target'
})

RainbowAura = AurasGroup:AddToggle('RainbowAura', {
    Text = 'Rainbow Aura',
    Default = false,
...
ToxicAura = AurasGroup:AddToggle('ToxicAura', {
    Text = 'Toxic Aura',
    Default = false,
    Tooltip = 'Automatically toxify blocks'
})

SignAura = AurasGroup:AddToggle('SignAura', {
    Text = 'Sign Aura',
    Default = false,
    Tooltip = 'Automatically place signs'
})

AnchorAura = AurasGroup:AddToggle('AnchorAura', {
    Text = 'Anchor Aura',
    Default = false,
    Tooltip = 'Automatically anchor blocks'
})

UnanchorAura = AurasGroup:AddToggle('UnanchorAura', {
    Text = 'Unanchor Aura',
    Default = false,
...
AuraRange = AuraSettingsGroup:AddSlider('AuraRange', {
    Text = 'Aura Range',
    Default = 100,
    Min = 5,
    Max = 100,
...
AuraSpeed = AuraSettingsGroup:AddSlider('AuraSpeed', {
    Text = 'Aura Speed',
    Default = 100,
    Min = 1,
    Max = 100,
...
local AuraBatch = AuraSettingsGroup:AddSlider('AuraBatch', {
    Text = 'Batch Size',
...
_enlBtn.Position = UDim2.fromOffset(10, 118)

local _enlHeaderGroup = _enlightenPopout:AddGroupbox('Status')
local _enlStatusLabel  = _enlHeaderGroup:AddLabel('Watching — no players found')
...
_bsToggleBtn.Position = UDim2.fromOffset(10, 84)   -- Servers(50) + 28h + 6gap = 84
...
local _bsNameInput = _bsUploadGroup:AddInput('_bsNameInput', {
    Default     = '',
    Numeric     = false,
    Finished    = true,
    Text        = 'Build Name:',
...
local _bsDescInput = _bsUploadGroup:AddInput('_bsDescInput', {
    Default     = '',
    Numeric     = false,
    Finished    = true,
    Text        = 'Description:',
...
local _bsImageIdInput = _bsUploadGroup:AddInput('_bsImageIdInput', {
    Default     = '',
    Numeric     = true,
    Finished    = true,
    Text        = 'Image ID:',
    Placeholder = 'Roblox asset ID for thumbnail (optional)...',
})

local _bsUploadLabel = _bsUploadGroup:AddLabel('Ready to upload')
...
_bsUploadGroup:AddButton({
    Text = 'Upload Server Blocks',
    Func = function()
        local arr = {}
...
local _bsStatusLabel = _bsBrowseGroup:AddLabel('Loading...')
_bsBrowseGroup:AddDivider()
...
_bsCtrlGroup:AddButton({
    Text = 'Clear Active Preview',
    Func = function()
...
local AutoR6Toggle = ToolsEtc:AddToggle('AutoR6Toggle', {
...
local DisableDeleteSoundButton = BuildServerGroup:AddButton({
    Text = 'Disable Delete Sound',
    Func = _requirePremium(function()
...
importJsonInput = BuildImportGroup:AddInput('ImportJSON', {
    Default     = '',
    Numeric     = false,
    Finished    = true,
    Text        = 'Paste JSON here:',
    Placeholder = 'Ctrl+V / use Paste button below',
...
_previewToggleBtn = BuildImportGroup:AddButton({
    Text = 'Preview',
    Func = _requirePremium(function()
...
_abfBtn.Position = UDim2.fromOffset(10, 152)

local _grpStatus = _abfPopout:AddGroupbox('Status')
_abfStatusLabel = _grpStatus:AddLabel("Status: Inactive")
_abfCountLabel  = _grpStatus:AddLabel("Deletions tracked: 0")
_abfLastLabel   = _grpStatus:AddLabel("Last alert: none")

local _grpSettings = _abfPopout:AddGroupbox('Detection Settings')
_grpSettings:AddToggle('ABFAutoFix', {
    Text = 'Auto-Fix on Detection', Default = true,
    Tooltip = 'Restore blocks automatically; alert popup still shows',
    Callback = function(v) ABF_AUTO_FIX = v end,
})
_grpSettings:AddSlider('ABFThreshold', {
    Text = 'Block Threshold', Default = 5, Min = 3, Max = 20, Rounding = 0,
    Callback = function(v) ABF_THRESHOLD = v end,
})
_grpSettings:AddSlider('ABFWindow', {
    Text = 'Detection Window (s)', Default = 2, Min = 1, Max = 6, Rounding = 0,
    Callback = function(v) ABF_WINDOW = v end,
})
_grpSettings:AddSlider('ABFMinTime', {
    Text = 'Min Victim Time (s)', Default = 60, Min = 0, Max = 600, Rounding = 0,
    Callback = function(v) ABF_MIN_TIME = v end,
})

local _grpCtrl = _abfPopout:AddGroupbox('Controls')
_grpCtrl:AddButton({ Text = 'Start Scanning', Func = _abfStart })
_grpCtrl:AddButton({ Text = 'Stop Scanning',  Func = _abfStop  })

local _grpAdmin = _abfPopout:AddGroupbox('Admin Tools')

-- "debug" toggles the build tools overlay that exposes stash/dupe exploits
_grpAdmin:AddButton({
    Text = 'Debug (anti-stash)',
    Func = function()
        pcall(function()
...
local _clearInvInput = _grpAdmin:AddInput('ABFClearInvTarget', {
    Default     = 'all',
    Numeric     = false,
    Finished    = true,
    Text        = 'Clearinv target:',
...
local _abfJsonInput = _grpImport:AddInput('ABFJson', {
    Default = '', Numeric = false, Finished = true,
    Text = 'Backup JSON:', Placeholder = 'Paste build JSON as restore reference...',
})
_grpImport:AddButton({
    Text = 'Load into Import Build Tab',
    Func = function()
...
_grpNoclip:AddToggle('NoclipActive', {
    Text = 'Enable Auto Noclip', Default = false,
...
local _noclipBlInput = _grpNoclip:AddInput('NoclipBlacklist', {
    Default = table.concat(_noclipBlacklist, ", "),
    Numeric = false, Finished = true,
    Text = 'Blacklist:', Placeholder = 'Comma-separated names/userids...',
})
_grpNoclip:AddButton({
    Text = 'Update Blacklist',
    Func = function()
        _noclipBlacklist = {}
...
local _quickCmdTarget = _grpQuickCmd:AddInput('QuickCmdTarget', {
    Default = '', Numeric = false, Finished = true,
    Text = 'Target player:', Placeholder = 'Player name or "all"',
})
...
_grpQuickCmd:AddButton({ Text = 'Give R6',      Func = function() _sendQuickCmd("r6") end })
_grpQuickCmd:AddButton({ Text = 'Give Fly',     Func = function() _sendQuickCmd("fly") end })
_grpQuickCmd:AddButton({ Text = 'Give Bkit',    Func = function() _sendQuickCmd("bkit") end })
_grpQuickCmd:AddButton({ Text = 'Give Carpet',  Func = function() _sendQuickCmd("carpet") end })
_grpQuickCmd:AddButton({ Text = 'Give Paint',   Func = function() _sendQuickCmd("paint") end })
_grpQuickCmd:AddButton({ Text = 'Give Boombox', Func = function() _sendQuickCmd("boombox") end })
_grpQuickCmd:AddButton({ Text = 'Give Noclip',  Func = function() _sendQuickCmd("noclip") end })
...
ToolsESPToggle = ToolESPGroup:AddToggle('ToolsESP', {
...
AutoRejoin = SettingsGroup:AddToggle('AutoRejoin', {
...
ShowCredits = SettingsGroup:AddToggle('ShowCredits', {
...
SettingsGroup:AddToggle('BoostFPS', {
    Text = 'Boost FPS',
    Default = false,
...
SettingsGroup:AddToggle('HideOwnTag', {
    Text    = 'Hide Own Tag',
    Default = false,
...
SettingsGroup:AddSlider('ServerHistoryCount', {
    Text     = 'Recent Server Memory',
    Default  = 3,
    Min      = 0,
    Max      = 20,
    Rounding = 0,
...
SettingsGroup:AddToggle('AutoExecOnHop', {
...
SettingsGroup:AddLabel('Script Version: 2.9')
SettingsGroup:AddLabel('Game: ' .. (productInfo and productInfo.Name or "Loading..."))
SettingsGroup:AddLabel('Place ID: ' .. game.PlaceId)
SettingsGroup:AddLabel('Player: ' .. plr.Name)
local StatusLabel = SettingsGroup:AddLabel('Status: ' .. (isOwner and 'OWNER 👑' or isPremium and 'PREMIUM ⭐' or 'USER 👥'))
...
local DecalToolButton = AdvancedGroup:AddButton({
    Text = 'Decal Tool',
    Func = _requirePremium(function()
...
local EditorToolButton = AdvancedGroup:AddButton({
    Text = 'Editor Tool',
    Func = _requirePremium(function()
...
StashPlatform:AddButton({
 ...
 PlayerToolsGroup = Tabs.StashSystem:AddRightGroupbox('Player Management')
...
PlayerToolsGroup:AddToggle('TouchFling', {
    Text = 'Touch Fling',
    Default = false,
...
FlyEnabled = CharacterGroup:AddToggle('FlyEnabled', {
    Text = 'Toggle Fly',
    Default = false,
    Tooltip = 'Toggle flying mode'
})

FlySpeed = CharacterGroup:AddSlider('FlySpeed', {
    Text = 'Fly Speed',
    Default = 50,
    Min = 10,
    Max = 200,
...
local NoclipBtn
NoclipBtn = CharacterGroup:AddButton({
    Text = 'Noclip: OFF',
    Func = function()
...
ChatSpyToggle = ChatGroup:AddToggle('ChatSpy', {
...
FakeTimeToggle = FakeTimeGroup:AddToggle('FakeTime', {
...
local DecimalDonateMode = FakeTimeGroup:AddDropdown('DecimalDonateMode', {
    Values = {
        "Random",
        "Others",
        "Everyone"
    ...
    Default = 1,
    Text = 'Decimal Donate Target',
    Tooltip = 'Who to send your decimal time to'
})

local CleanDecimalTimeButton = FakeTimeGroup:AddButton({
    Text = 'Clean Decimal Time',
    Func = _requirePremium(function()
        local ls = plr:FindFirstChild("leaderstats")
...
local DayNightGroup = Tabs.Chat:AddLeftGroupbox('Day/Night Cycle')

local DayNightSpeed = DayNightGroup:AddSlider('DayNightSpeed', {
    Text    = 'Cycle Speed',
    Default = 0.5,
    Min     = 0.1,
    Max     = 5,
...
local DayNightToggle = DayNightGroup:AddToggle('DayNightCycle', {
...
AntiGravity = AntisGroup:AddToggle('AntiGravity', {
    Text = 'Anti Gravity',
    Default = false,
...
local PatternsGroup  = Tabs.Parts:AddLeftGroupbox('Patterns')
local PartsSetGroup  = Tabs.Parts:AddRightGroupbox('Settings & Advanced')
...
PatternsGroup:AddDropdown('pcPattern', {
    Text    = 'Pattern',
    Values  = { 'Origin', 'Pythagoras Ring', 'Sigma Orbit', 'Alpha Wave', 'Pi Tilt', 'Pi Squared', 'Two Circles', 'Bloom Cage', 'Square²' },
    Default = 'Origin',
...
PatternsGroup:AddDivider()
PatternsGroup:AddSlider('pcRingSize', {
    Text = 'Ring Size', Min = 0, Max = 150, Default = 10, Rounding = 1,
}):OnChanged(function(v) _pcRingSize = v end)

PartsSetGroup:AddSlider('pcPickRad', {
    Text = 'Pick Up Radius', Min = 0, Max = 2000, Default = 500, Rounding = 1,
}):OnChanged(function(v) _pcPickRad = v end)

PartsSetGroup:AddSlider('pcSpeed', {
    Text = 'Speed', Min = 1, Max = 500, Default = 10, Rounding = 1,
}):OnChanged(function(v) _pcSpeed = v end)

PartsSetGroup:AddToggle('pcUseCFrame', { Text = 'Use CFrame', Default = false }):OnChanged(function(v) _pcUseCFrame = v end)
PartsSetGroup:AddToggle('pcNoclip',    { Text = 'Noclip',     Default = false }):OnChanged(function(v) _pcNoclip = v end)
PartsSetGroup:AddToggle('pcAutoGrab',  { Text = 'Auto Grab All Objects', Default = false }):OnChanged(function(v) _pcAutoGrab = v end)

PartsSetGroup:AddSlider('pcAutoRad', {
    Text = 'Auto Grab Radius', Min = 100, Max = 2000, Default = 100, Rounding = 1,
}):OnChanged(function(v) _pcAutoRad = v end)

PartsSetGroup:AddToggle('pcEntropy',  { Text = 'Entropy (Lag Mode)', Default = false }):OnChanged(function(v) _pcEntropy = v end)
PartsSetGroup:AddToggle('pcRealLag',  { Text = 'Real Lag (1000 Up)', Default = false }):OnChanged(function(v) _pcRealLag = v end)
PartsSetGroup:AddToggle('pcDetailed', { Text = 'Detailed Blocks',    Default = false })
PartsSetGroup:AddToggle('pcRedColor', { Text = 'Red Color',          Default = false }):OnChanged(function(v) _pcRedColor = v end)

PartsSetGroup:AddDivider()
PartsSetGroup:AddToggle('pcChaos',   { Text = 'Chaos (Fling)',          Default = false }):OnChanged(function(v) _pcChaos = v end)

PartsSetGroup:AddDivider()
PartsSetGroup:AddButton({
    Text = 'Grab Nearby Parts',
    Func = function()
...
PartsSetGroup:AddToggle('pcActive', {
...
local function applyMenuSize(scaleValue)
    local _holder = Window and Window.Holder or _mainFrame
    if not (_holder and _holder.Parent) then return end
    _mainFrame = _holder
    local _scale = (tonumber(scaleValue) or 10) / 10
    _holder.Size = windowsize
    if Window and Window.SetScale then
        Window:SetScale(_scale)
    else
        _holder.Size = muludim2(divudim2(windowsize, 10), tonumber(scaleValue) or 10)
    end
end
...
task.defer(applyMenuSize, MenuSize.Value)
